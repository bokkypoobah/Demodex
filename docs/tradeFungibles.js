...
data: {{ data }}
  ...
  computed: {
    ...
    newSellOffers() {
      ...
      for (const [i, price] of prices.entries()) {
        ...
        if (price.valid) {
          ...
          if (maxWeth != null) {
            // wethModDecimals
            const mod = BigInt(maxWeth.toString()) * BigInt(TENPOW18.toString()) % (BigInt(price.price.toString()) * BigInt(10**wethModDecimals));
            console.log("SELL mod: " + ethers.utils.formatEther(mod) + " " + mod.toString());
            //
            // // tokens = weth x 10^18 / price
            // tokensFromMaxWeth = maxWeth.mul(TENPOW18).sub(mod).div(price_);
            const tokensFromMaxWeth = maxWeth.mul(TENPOW18).sub(mod).div(price.price);
        ...
        records.push({ ...price, originalTokens: price.tokens, tokens: tokens.toString(), totalTokens: totalTokens.toString(), weths: weths != null && weths.toString() || null, totalWeths: totalWeths.toString(), _rowVariant: price.txHash == null ? 'danger' : (this.indexToAddress[price.maker] == this.coinbase ? 'success' : null) });
      }
    ...
    newBuyOffers() {
      ...
      console.log("  #                BUY Price      Maker                   Tokens             WETH Balance            WETH Approval           Tokens in WETH           Rounded Tokens")
      console.log("--- ------------------------ ---------- ------------------------ ------------------------ ------------------------ ------------------------ ------------------------")
      const records = [];
      const trades = [];
      let totalTokens = ethers.BigNumber.from(0);
      ...
      for (const [i, price] of prices.entries()) {
        ...
        let tokens = ethers.BigNumber.from(price.tokens);
        let price_ = ethers.BigNumber.from(price.price);
        let weths = null;
        // if (price.txHash == null) {
        //   console.log("SIMULATED prices[" + i + "]: " + JSON.stringify(price));
        // } else if (!price.valid) {
        ...
        const tokensInWeth = ethers.BigNumber.from(price.tokens).mul(ethers.BigNumber.from(price.price)).div(TENPOW18);
        const roundedTokens = tokensInWeth.mul(TENPOW18).div(ethers.BigNumber.from(price.price));

        // let // availableTokens = price.tokens;
        // availableTokens = roundedTokens;
        console.log(
          padLeft(i + 1, 3) + " " +
          padLeft(ethers.utils.formatEther(price_), 24) + " " +
          this.indexToAddress[price.maker].substring(2, 12) + " " +
          padLeft(ethers.utils.formatUnits(price.tokens, this.tokenSet.decimals), 24) + " " +
          padLeft(ethers.utils.formatUnits(wethBalance, 18), 24) + " " +
          padLeft(ethers.utils.formatUnits(wethApproval, 18), 24) + " " +
          padLeft(ethers.utils.formatUnits(tokensInWeth, 18), 24) + " " +
        ...
        if (price.valid) {
          ...
          if (maxWeth != null) {
            ...
            // wethModDecimals
            const mod = BigInt(maxWeth.toString()) * BigInt(TENPOW18.toString()) % (BigInt(price_.toString()) * BigInt(10**wethModDecimals));
            console.log("BUY mod: " + ethers.utils.formatEther(mod) + " " + mod.toString());
